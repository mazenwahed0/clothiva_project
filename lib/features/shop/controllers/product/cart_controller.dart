...
class CartController extends GetxController {
  ...
  void updateAlreadyAddedProductCount(ProductModel product){
    if(product.productType == ProductType.single.toString()) {
      ...
      if(variationId.isNotEmpty) {
        productQuantityInCart.value = getVariationProductQuantityInCart(product.id, variationId);
      } else {
  ...
  int getProductQuantityInCart(String productId) {
    final foundItem = cartItems.where(
      (item) => item.productId == productId).fold(0, (previousValue, element) => previousValue + element.quantity);
      return foundItem;
  }
