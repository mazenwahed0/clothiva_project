...
class WishlistRepository extends GetxController {
  ...
  Future<void> _createWishlistForOwner(
      String ownerId, List<String> productIds) async {
    try {
  ...
  Future<void> addCollaboratorToOwner(
      String ownerId, String collaboratorId) async {
    try {
  ...
  Future<void> removeCollaboratorFromOwner(
    ...
    try {
      ...
      final docRef =
          _db.collection(CKeys.wishlistCollection).doc(query.docs.first.id);
      await docRef.update({
      ...
      final sharedWith =
          List<String>.from(reloaded.data()?['sharedWith'] ?? []);
...
//         await _db.collection(CKeys.wishlistCollection).doc(doc.id).update({
//           'isShared': shareMode,
//           'sharedWith': shareMode ? doc['sharedWith'] : [],
//           'updatedAt': FieldValue.serverTimestamp(),
//         });
//       }
//     } on FirebaseException catch (e) {
...
// //   /// Return the first wishlist that the user either owns or is a collaborator of.
// //   Future<WishlistModel?> getUserWishlist() async {
// //     try {
// //       if (currentUser == null) return null;

// //       final uid = currentUser!.uid;

// //       // Find wishlist where user is owner OR sharedWith contains user
// //       final query = await _db
// //           .collection(CKeys.wishlistCollection)
// //           .where(Filter.or(
// //             Filter.and(
// //               Filter('ownerId', isEqualTo: uid),
// //               Filter('isShared', isEqualTo: true),
// //             ),
// //             Filter('sharedWith', arrayContains: uid),
// //           ))
// //           .limit(1)
...
// //       print(' Toggling product: $productId');
// //       final wishlist = await getUserWishlist();
// //       print(' Wishlist found? ${wishlist != null}');

// //       if (wishlist == null) {
// //         // create with single product
// //         await _createWishlistForOwner(currentUser!.uid, [productId]);
// //         return;
// //       }
// //       print('🟢 Updating existing wishlist: ${wishlist.id}');
...
// //       if (query.docs.isEmpty) {
// //         // create wishlist for owner with no products yet
// //         await _db.collection(CKeys.wishlistCollection).add({
// //           'ownerId': ownerId,
// //           'productIds': [],
// //           'sharedWith': [collaboratorId],
...
// //       // Optionally set isShared=false if sharedWith becomes empty
// //       final reloaded = await docRef.get();
// //       final sharedWith =
// //           List<String>.from(reloaded.data()?['sharedWith'] ?? []);
// //       if (sharedWith.isEmpty) {
